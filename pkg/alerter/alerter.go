...
// Read summary protos and call functions for each Klaxon on certain conditions
// Maintains a copy of summary protos to recognize last read state after reboot
func Update(ctx context.Context, client *storage.Client, path gcs.Path, concurrency int, dashboard string, subscribers []Klaxon, confirm bool) error {
	if concurrency < 1 {
		return fmt.Errorf("concurrency must be positive, got: %d", concurrency)
	}

	if len(subscribers) == 0 {
		logrus.Warn("No subscribers to alerter; events will drop silently")
	}
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			for dash := range dashboards {
				...
				newPath, err := path.ResolveReference(&url.URL{Path: summaryPath(dash.Name)})
				if err != nil {
				...
				if oldSummary != nil {
					callOutDifferences(ctx, newSummary, oldSummary, dash, subscribers)
				}

				// Write the new summary to where the old summary was
				err = summarizer.WriteSummary(ctx, client, *oldPath, newSummary)
				if err != nil {
...
// A normalized path for a summary file is "prefix-dashboardnameinlowercase"
...
// TODO(chases2): Move this and WriteSummary into a separate file in summarizer package
...
func callOutDifferences(ctx context.Context, new, old *summarypb.DashboardSummary, config *configpb.Dashboard, subscribers []Klaxon) {
	...
	for _, dashboardTab := range config.DashboardTab {
		...
		switch newSummary.OverallStatus {
		case summarypb.DashboardTabSummary_PASS:
			for _, sub := range subscribers {
				err := sub.OnEachSuccess(ctx, newSummary, oldSummary, dashboardTab)
				if err != nil {
					logrus.Errorf("OnEachSuccess Dashboard %s Error: %v", dashboardTab.Name, err)
				}
			}
		case summarypb.DashboardTabSummary_FAIL:
			for _, sub := range subscribers {
				err := sub.OnEachFailure(ctx, newSummary, oldSummary, dashboardTab)
				if err != nil {
					logrus.Errorf("OnEachFailure Dashboard %s Error: %v", dashboardTab.Name, err)
				}
			}
		case summarypb.DashboardTabSummary_STALE:
			if oldSummary.OverallStatus != summarypb.DashboardTabSummary_STALE {
				for _, sub := range subscribers {
					err := sub.OnBecomingStale(ctx, newSummary, oldSummary, dashboardTab)
					if err != nil {
		...
		for _, failingTest := range newSummary.FailingTestSummaries {
			if failingTest.FailCount == alertThreshold {
				...
				if maySendAlert {
					for _, sub := range subscribers {
						err := sub.OnConsistentFailure(ctx, newSummary, oldSummary, dashboardTab)
						if err != nil {
